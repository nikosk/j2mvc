...
public class HTMLSanitiser {
    ...
    private static final Set<String> VALID_ATTRIBUTE_NAMES = new HashSet<String>(Arrays.asList(new String[]{
                "id", "class", "href", "target", "title", "name", "alt", "style", "src", "width", "height", "value","type"
            }));
    ...
    private static String sanitise(String pseudoHTML, boolean formatWhiteSpace, boolean stripInvalidElements) {
        ...
        for (Tag tag : tags) {
            ...
            //reencodeTextSegment(source, outputDocument, pos, tag.getBegin(), formatWhiteSpace); // encoding breaks greek charecters
            pos = tag.getEnd();
        }
        //reencodeTextSegment(source, outputDocument, pos, source.getEnd(), formatWhiteSpace);// encoding breaks greek charecters
        return outputDocument.toString();
    }

    private static boolean processTag(Tag tag, OutputDocument outputDocument) {
        String elementName = tag.getName();
        elementName = elementName == null ? elementName : elementName.toLowerCase();
